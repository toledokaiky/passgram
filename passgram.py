...
def custom_proxy(verbose, file_proxies, proxy_limit):
    ...
    printit(
        str(len(proxies)),
        coledt=[1, 49, 96],
        normaltxt_start="[+] Total de proxies: ",
    )
    printit(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="(s41r4j:igbf)> Instagram Brute Forcer | Manual de Uso"
    )
    ...
    if proxy_file:
        ...
        try:
            ...
            printit(f"[!] {proxy_file} não encontrado", coledt=[4, 49, 91])
            raise KeyboardInterrupt()
    else:
    ...
    if username is None or wordlist is None:
        ...
        printit(
            "[-] `[-] `-u/--usuario` e `-w/--wordlist` são obrigatorios",
            coledt=[1, 49, 91],
            line_down=True,
        )
        sys.exit()
    ...
    try:
        ...
        printit("[!] Wordlist não funciona, use um .txt", coledt=[4, 49, 91])
        sys.exit()
